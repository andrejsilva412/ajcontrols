...
type
  ...
  TAJValorPorExtenso = class(TComponent)
  private
    FValor: Extended;
    FTexto: String;
...
constructor TAJValorPorExtenso.Create(Owner: TComponent);
begin
...
{ altera o valor }
procedure TAJValorPorExtenso.MudaValor(V: Extended);
begin
  if V <> FValor then
  begin
    if V < 0.0 then
...
procedure TAJValorPorExtenso.MudaMoedaNoSingular(M: String);
begin
...
procedure TAJValorPorExtenso.MudaMoedaNoPlural(M: String);
begin
...
procedure TAJValorPorExtenso.MudaLabelAssociado(Lbl: TLabel);
begin
...
{ muda tipo texto }
procedure TAJValorPorExtenso.MudaTipoTexto(TipoTxt: TTipoTexto);
begin
...
procedure TAJValorPorExtenso.Notification(AComponent: TComponent;
...
function TAJValorPorExtenso.TrataGrupo(const S: String): String;
const
...
{ atualiza o texto }
function TAJValorPorExtenso.PorExtenso: String;
var
  Lst: TStringList;
  I: Integer;
  Aux: String;
...
procedure Register;
begin
  RegisterComponents('AJCtrls Standard', [TAJValorPorExtenso]);
end;
