...
uses
  LMessages, LCLIntf, LCLType, Classes, Controls, Graphics,
  Types, Forms, Dialogs, Math, SysUtils;

type
  TBtnState = (bsNormal, bsOver, bsDown);
  TButtonLayout = (blGlyphLeft, blGlyphRight, blGlyphTop, blGlyphBottom);
  TColorScheme = (csNeoDesert, csNeoSky, csNeoGrass, csNeoSilver,
       csNeoRose, csNeoSun, csCustom);

  { TAJButton }

  TAJButton = class(TCustomControl)
  private
    FColorFace: TColor;
    FColorGrad: TColor;
    FColorBorder: TColor;
...
procedure Register;
begin
  RegisterComponents('AJCtrls', [ TAJButton]);
end;

constructor TAJButton.Create(AOwner: TComponent);
begin
  inherited;
  Width:= 75;
  Height:= 25;
...
procedure TAJButton.MouseEnter(var msg: TMsg);
begin
...
procedure TAJButton.MouseLeave(var msg: TMsg);
begin
  inherited;
  FBtnState:= bsNormal;
  Paint;
...
procedure TAJButton.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TAJButton.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  inherited;
  FClicked:= False;
...
procedure TAJButton.MouseMove (Shift: TShiftState; X, Y: Integer);
begin
  inherited;
end;

procedure TAJButton.WMSetFocus(var msg: TLMSetFocus);
begin
  FFocused:= true;
  Paint;
end;

procedure TAJButton.WMKillFocus(var msg: TLMKillFocus);
begin
  FFocused:= false;
  FBtnState:= bsNormal;
  Paint;
...
procedure TAJButton.WMKeyDown(var msg: TLMKeyDown);
begin
...
procedure TAJButton.WMKeyUp(var msg: TLMKeyUp);
begin
...
procedure TAJButton.CMTextChanged (var msg: TMsg);
begin
  Invalidate;
end;

procedure TAJButton.SetCtl3D(Value: Boolean);
begin
  FCtl3D:= Value;
  Invalidate;
end;

procedure TAJButton.SetLayout(Value: TButtonLayout);
begin
  FLayout:= Value;
  Invalidate;
end;

procedure TAJButton.SetGlyph(Value: TBitmap);
begin
  FGlyph.Assign(Value);
  Invalidate;
end;

procedure TAJButton.SetSpacing(Value: Integer);
begin
  FSpacing:= Value;
  Invalidate;
end;

procedure TAJButton.SetTransparentGlyph(Value: Boolean);
begin
  FTransparentGlyph:= Value;
  Invalidate;
end;

procedure TAJButton.SetGradient(Value: Boolean);
begin
  FGradient:= Value;
  Invalidate;
end;

procedure TAJButton.CMFontChanged(var msg: TMsg);
begin
  Invalidate;
end;

procedure TAJButton.CMDialogKey(var msg: TCMDialogKey);
begin
  with msg do begin
...
procedure TAJButton.CMEnabledChanged(var msg: TMsg);
begin
  inherited;
  Invalidate;
end;

procedure TAJButton.CMDialogChar(var msg: TCMDialogChar);
begin
  with msg do
...
procedure TAJButton.SetModalResult(Value: TModalResult);
begin
  FModalResult:= Value;
end;

procedure TAJButton.SetCancel(Value: Boolean);
begin
  FCancel:= Value;
end;

procedure TAJButton.SetDefault(Value: Boolean);
var
  Form: TCustomForm;
begin
  FDefault := Value;
...
procedure TAJButton.SetHotTrack(Value: Boolean);
begin
  FHotTrack:= Value;
  Invalidate;
end;

procedure TAJButton.SetColors(Index: integer; Value: TColor);
begin
  case Index of
    0: FColorFace:= Value;
    1: FColorGrad:= Value;
...
procedure TAJButton.SetColorScheme(Value: TColorScheme);
begin
  FColorScheme:= Value;
...
procedure TAJButton.GradientFillRect(Canvas: TCanvas; Rect: TRect;
                StartColor, EndColor: TColor);
var
  Steps: Integer;
...
procedure TAJButton.SetAutoSize(Value: Boolean);
begin
  if Value then
    DoAutoSize;
end;

function TAJButton.GetColorScheme: TStringList;
begin
