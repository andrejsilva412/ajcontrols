...
uses
  LCLIntf, Classes, Controls, Graphics, Messages, Types, Forms, Dialogs,
...
procedure Register;
begin
  RegisterComponents('AJCtrls', [TAJPanel]);
end;

constructor TAJPanel.Create(AOwner: TComponent);
begin
  inherited;
  FColorDark:= clBlack;
...
procedure TAJPanel.DrawBevel(Canvas: TCanvas; Rect: TRect; LightColor, DarkColor: TColor);
begin
  Canvas.Pen.Width:= 1;
...
procedure TAJPanel.DrawFrame(Canvas: TCanvas; Offset, FrameWidth: Integer; LightColor, DarkColor: TColor);
var
  i: integer;
begin
...
procedure TAJPanel.SetColors(Index: Integer; Value: TColor);
begin
  case Index of
    0: FColorFace:= Value;
    1: FColorGrad:= Value;
...
procedure TAJPanel.SetFillDirection(Value: TFillDirection);
begin
  FFillDirection:= Value;
  Invalidate;
end;

procedure TAJPanel.SetColorScheme(Value: TColorScheme);
begin
  FColorScheme:= Value;
...
procedure TAJPanel.GradientFillRect(Canvas: TCanvas; Rect: TRect;
                StartColor, EndColor: TColor; Direction: TFillDirection);
var
  Steps: Integer;
