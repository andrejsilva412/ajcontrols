...
uses
  {$ifdef windows} Windows, ComObj, {$endif} InterfaceBase, Forms, Types,
...
begin
  ...
  begin
    ...
    Result := GlobalTaskBarInterface.SetProgressState(WidgetSet.AppHandle, Flag) = S_OK;
  end;
end;
...
begin
  ...
  if CheckAPI then
  begin
    Result := GlobalTaskBarInterface.SetProgressValue(WidgetSet.AppHandle, ACurrent, AMax) = S_OK;
  end;
...
begin
  ...
  if CheckAPI then
  begin
    Result := GlobalTaskBarInterface.SetOverlayIcon(WidgetSet.AppHandle, AIcon, PAnsiChar(ADescription)) = S_OK;
  end;
end;
